...
const AppointmentsContainer = () => {
  ...
  const { data, loading, error } = useSelector((state: any) => state.allAppointments);
  ...
  const handleAccept = (appointmentId: number) => {
    ...
    setAppointments(prevAppointments => prevAppointments.map(appointment => appointment.appointment_id === appointmentId ? { ...appointment, status: "approved" } : appointment));
  };

  const handleReject = (appointmentId: number) => {
    dispatch(updateApppointment({ appointmentId, status: "rejected" }));
    toast.error("Appointment Rejected");
    setAppointments(prevAppointments => prevAppointments.map(appointment => appointment.appointment_id === appointmentId ? { ...appointment, status: "rejected" } : appointment));
  };
  ...
  const filteredAppointments = selectedStatus === "all" ? appointments : appointments.filter(appointment => appointment.status.toLowerCase() === selectedStatus);
    ...
    <div className="container mx-auto p-4 mt-20">
      ...
      <div className="sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredAppointments.map(appointment => (
          <AppointmentCard key={appointment.appointment_id} appointment={appointment} onAccept={handleAccept} onReject={handleReject} getStatusColor={getStatusColor} formatTime={formatTime} />
        ))}
      </div>
    </div>
  );
